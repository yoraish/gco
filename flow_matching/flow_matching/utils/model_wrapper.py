...
class ModelWrapperCoGen(ABC, nn.Module):
    """
    This class is used to wrap around a model that takes both continuous and discrete inputs.
    """
    ...
    def forward(self, x_d: Tensor, x_c: Tensor, t: Tensor, **extras) -> Tuple[Tensor, Tensor]:
        """
        This method defines how inputs should be passed through the wrapped model.
        Here, we're assuming that the wrapped model takes both :math:`x_d` and :math:`x_c` as input,
        along with any additional keyword arguments.

        Args:
            x_d (Tensor): discrete input data to the model (batch_size, ...).
            x_c (Tensor): continuous input data to the model (batch_size, ...).
            t (Tensor): time (batch_size).
            **extras: additional information forwarded to the model, e.g., text condition.

        Returns:
            Tuple[Tensor, Tensor]: model output. Logits for the discrete part, and velocity for the continuous part.
        """
        return self.model(x_d=x_d, x_c=x_c, t=t, **extras)
